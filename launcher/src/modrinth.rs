...
const USER_AGENT_VALUE: &str = "velgrinor-launcher/1.0";
...
impl ModrinthClient {
    ...
    pub fn get_projects(&self, ids: &[String]) -> Result<Vec<Project>> {
        if ids.is_empty() {
            return Ok(Vec::new());
        }
        let encoded_ids = serde_json::to_string(ids).context("failed to encode project ids")?;
        let url = format!(
            "{}/projects?ids={}",
            API_BASE,
        ...
        self.client
            .get(&url)
            .send()
            .context("failed to fetch Modrinth projects")?
            .error_for_status()
            .context("Modrinth projects request failed")?
            .json()
            .context("failed to parse Modrinth projects")
    }
    ...
    pub fn get_versions(&self, version_ids: &[&str]) -> Result<Vec<Version>> {
        ...
        let ids_json =
            serde_json::to_string(version_ids).context("failed to serialize version IDs")?;
        let url = format!(
            "{}/versions?ids={}",
            API_BASE,
