...
fn populate_dir(
    ...
    let default_ext = match kind {
        ContentKind::Mod => "jar",
        ContentKind::ModPack | ContentKind::ResourcePack | ContentKind::ShaderPack => "zip",
    ...
    #[cfg(not(windows))]
    {
        if let Err(err) = fs::hard_link(src, dst) {
            fs::copy(src, dst).with_context(|| {
                format!(
                    "failed to copy {} to {} after hardlink error: {err}",
                    src.display(),
                    dst.display()
                )
            })?;
...
mod tests {
    ...
    fn test_paths() -> Paths {
        let unique = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let base = std::env::temp_dir().join(format!("velgrinor-instance-test-{unique}"));

        Paths {
            store_mods: base.join("store").join("mods").join("sha256"),
            store_modpacks: base.join("store").join("modpacks").join("sha256"),
    ...
    #[cfg(unix)]
    #[test]
    fn replaces_legacy_symlinks_with_regular_files() {
        use std::os::unix::fs::symlink;

        let paths = test_paths();
        paths.ensure().unwrap();
        fs::write(paths.store_mod_path("modhash"), "mod bytes").unwrap();
        let legacy_dir = paths.instance_dir("profile").join("mods");
        fs::create_dir_all(&legacy_dir).unwrap();
        symlink(
            paths.store_mod_path("modhash"),
            legacy_dir.join("legacy.jar"),
        )
        .unwrap();
        let profile = Profile {
            id: "profile".to_string(),
        ...
        let instance = materialize_instance(&paths, &profile).unwrap();
        let metadata = fs::symlink_metadata(instance.join("mods/mod.jar")).unwrap();
        assert!(!metadata.file_type().is_symlink());
        assert!(!instance.join("mods/legacy.jar").exists());
    }
}
