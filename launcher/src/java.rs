...
fn parse_java_version_output(output: &str) -> Result<JavaVersionInfo> {
    ...
    let version = extract_version_string(version_line).context("Could not parse Java version")?;
...
fn collect_common_candidates(candidates: &mut Vec<PathBuf>) {
    ...
    if let Ok(home) = std::env::var("HOME") {
        ...
        if asdf_dir.exists()
            && let Ok(entries) = std::fs::read_dir(&asdf_dir)
        {
            for entry in entries.flatten() {
                let java_path = entry.path().join("bin").join(java_executable_name());
                candidates.push(java_path);
...
pub fn fetch_adoptium_release(java_major: u32) -> Result<AdoptiumRelease> {
    ...
    let releases: Vec<Value> = resp.json().context("failed to parse Adoptium response")?;

    let release = releases
        .first()
        .context("no releases found for this Java version")?;

    let binary = release.get("binary").context("no binary info in release")?;

    let package = binary.get("package").context("no package info in binary")?;

    let version_data = release
        .get("version")
        .context("no version info in release")?;

    let semver = version_data
        .get("semver")
        .and_then(|v| v.as_str())
        .unwrap_or("unknown");
...
pub fn download_and_install_java(
    ...
    let manager = DownloadManager::new(install_dir.join("downloads.json"), 3)?;
    download_and_install_java_managed(java_major, install_dir, &manager, progress_callback)
}

pub fn download_and_install_java_managed(
    java_major: u32,
    install_dir: &Path,
    ...
    fs::create_dir_all(install_dir).context("failed to create Java install directory")?;

    // Download the archive
    let archive_path = install_dir.join(&release.filename);
    let mut request = DownloadRequest::new(&release.download_url, &archive_path);
    request.expected_size = (release.size > 0).then_some(release.size);
    request.sha256 = release.checksum.clone();
    request.group = Some(format!("java-{}", release.major));
    request.label = Some(release.filename.clone());
    manager.run_requests(vec![request], |snapshot| {
        if let Some(callback) = progress_callback.as_ref() {
            callback(snapshot.downloaded, snapshot.total.unwrap_or(release.size));
        }
    })?;
...
fn extract_zip(archive_path: &Path, dest_dir: &Path) -> Result<PathBuf> {
    ...
    for i in 0..archive.len() {
        ...
        if file.name().ends_with('/') {
            ...
            let mut outfile =
                fs::File::create(&outpath).context("failed to create file from zip")?;
...
fn find_java_in_extracted(jdk_dir: &Path) -> Result<PathBuf> {
    ...
    anyhow::bail!(
        "could not find java executable in extracted JDK at {}",
        jdk_dir.display()
    )
}
    ...
    if let Ok(entries) = fs::read_dir(&runtime_dir) {
        for entry in entries.flatten() {
            if entry.path().is_dir()
                && let Ok(java_path) = find_java_in_extracted(&entry.path())
            {
                ...
                if let Ok(inner_entries) = fs::read_dir(entry.path()) {
                    for inner in inner_entries.flatten() {
                        if inner.path().is_dir()
                            && let Ok(java_path) = find_java_in_extracted(&inner.path())
...
mod tests {
    ...
    fn test_detect_vendor() {
        assert_eq!(
            detect_vendor("OpenJDK Runtime Environment Temurin-17.0.2+8"),
            Some("Eclipse Temurin".to_string())
        );
        assert_eq!(
            detect_vendor("OpenJDK Runtime Environment Zulu17.32+13-CA"),
            Some("Azul Zulu".to_string())
        );
        assert_eq!(
            detect_vendor("OpenJDK Runtime Environment Corretto-17.0.2.8.1"),
            Some("Amazon Corretto".to_string())
        );
        assert_eq!(
            detect_vendor("openjdk version \"17.0.2\""),
            Some("OpenJDK".to_string())
        );
        assert_eq!(
            detect_vendor("Java(TM) SE Runtime Environment"),
            Some("Oracle".to_string())
        );
    }
}
