...
use crate::store::{ContentKind, hash_file, normalize_hash};
...
impl LibraryContentType {
    ...
    pub fn from_str(s: &str) -> Option<Self> {
        match s.to_lowercase().as_str() {
            ...
            "modpack" | "modpacks" => Some(Self::ModPack),
...
impl Library {
    ...
    pub fn add_item(&self, input: &LibraryItemInput) -> Result<LibraryItem> {
        ...
        let content_type = input
            .content_type
            .as_ref()
            .and_then(|value| LibraryContentType::from_str(value))
    ...
    pub fn enrich_item_from_content_ref(
        ...
        let inferred = source.and_then(modrinth_source);
        let platform = platform.or(inferred.as_ref().map(|_| "modrinth"));
        let project_id = project_id.or(inferred.as_ref().map(|(project, _)| project.as_str()));
        let version = version.or(inferred.as_ref().map(|(_, version)| version.as_str()));
    ...
    pub fn list_items(&self, filter: &LibraryFilter) -> Result<Vec<LibraryItem>> {
        ...
        if let Some(content_type) = &filter.content_type
            && let Some(ct) = LibraryContentType::from_str(content_type)
        {
    ...
    pub fn add_tag_to_item(&self, item_id: i64, tag_name: &str) -> Result<()> {
        // Ensure tag exists
        let tag = self.create_tag(tag_name, None)?;
    ...
    fn is_importable_file(&self, path: &Path, content_type: LibraryContentType) -> bool {
        ...
        match content_type {
            ...
            LibraryContentType::ModPack => {
                matches!(ext.as_deref(), Some("mrpack") | Some("zip"))
            }
    ...
    fn content_store_path(
        ...
        match content_type {
            ...
            LibraryContentType::ModPack => paths.store_modpack_path(hash),
    ...
    pub fn stats(&self) -> Result<LibraryStats> {
        ...
        let modpacks_count: u32 = self.conn.query_row(
            "SELECT COUNT(*) FROM library_items WHERE content_type = 'modpack'",
            [],
            |row| row.get(0),
        )?;
    ...
    pub fn sync_with_store(&self, paths: &Paths) -> Result<ImportResult> {
        ...
        for (store_dir, content_type) in [
            ...
            (&paths.store_modpacks, LibraryContentType::ModPack),
    ...
    pub fn get_unused_items(&self) -> Result<UnusedItemsSummary> {
        ...
        for row in rows {
            ...
            match item.content_type {
                ...
                LibraryContentType::ModPack => summary.modpacks.push(item),
    ...
    pub fn purge_unused_items(
        ...
        let items_to_delete: Vec<UnusedItem> = if content_types.is_empty() {
            ...
            for ct in content_types {
                match ct {
                    ...
                    LibraryContentType::ModPack => items.extend(unused.modpacks.clone()),
        ...
        for item in items_to_delete {
            ...
            if delete_files {
                ...
                if store_path.exists()
                    && let Err(e) = fs::remove_file(&store_path)
                {
                    result
                        .errors
                        .push(format!("Failed to delete {}: {}", item.name, e));
                    continue;
                }
            }
            ...
            match self.delete_item(item.id) {
                ...
                Ok(false) => {
                    result
                        .errors
                        .push(format!("Item {} not found in database", item.name));
                }
                Err(e) => {
                    result.errors.push(format!(
                        "Failed to delete {} from library: {}",
                        item.name, e
                    ));
                }
            }
...
fn modrinth_source(url: &str) -> Option<(String, String)> {
    let tail = url.split_once("cdn.modrinth.com/data/")?.1;
