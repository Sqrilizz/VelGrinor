...
impl Paths {
    pub fn new() -> Result<Self> {
        let mut base = if let Some(value) = nonempty_env_path("VELGRINOR_HOME") {
            value
        } else if let Some(value) = nonempty_env_path("SHARD_HOME") {
            value
        } else {
            let home = dirs::home_dir().context("could not determine home directory")?;
            let velgrinor_home = home.join(".velgrinor");
            let shard_home = home.join(".shard");
            if velgrinor_home.exists() || !shard_home.exists() {
                velgrinor_home
            } else {
                shard_home
            }
        ...
        let store_modpacks = base.join("store").join("modpacks").join("sha256");
    ...
    pub fn ensure(&self) -> Result<()> {
        ...
        std::fs::create_dir_all(&self.store_modpacks)
            .context("failed to create store/modpacks directory")?;
    ...
    pub fn store_modpack_path(&self, hash_hex: &str) -> PathBuf {
        self.store_modpacks.join(hash_hex)
    }
    ...
    pub fn downloads_state(&self) -> PathBuf {
        self.config.with_file_name("downloads.json")
    }
}
