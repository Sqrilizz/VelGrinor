...
use velgrinor::accounts::{
    create_offline_account, delete_account_tokens, export_accounts_to_file,
    import_accounts_from_file, load_accounts, merge_accounts, remove_account, save_accounts,
    set_active, upsert_account,
};
use velgrinor::auth::request_device_code;
use velgrinor::config::{load_config, save_config};
use velgrinor::content_store::{ContentStore, ContentType, Platform, SearchOptions};
use velgrinor::curseforge_pack::{import_curseforge_zip, is_curseforge_zip};
use velgrinor::dependencies::resolve_store_install_plan;
use velgrinor::diagnostics::{DiagnosticSeverity, diagnose_profile};
use velgrinor::download::{DownloadManager, DownloadRequest};
use velgrinor::instance::materialize_instance;
use velgrinor::library::{Library, LibraryContentType, LibraryFilter, LibraryItemInput};
use velgrinor::logs::{
    LogLevel, filter_by_level, format_entry, list_crash_reports, list_log_files, read_log_file,
    read_log_tail, search_logs, watch_log,
};
use velgrinor::minecraft::{launch, prepare};
use velgrinor::modpack::import_mrpack;
use velgrinor::ops::{
    finish_device_code_flow, parse_loader, resolve_input, resolve_launch_account,
};
...
use velgrinor::profile::{
    ...
    list_profiles, load_profile, remove_mod, remove_resourcepack, remove_shaderpack,
    rename_profile, save_profile, upsert_mod, upsert_resourcepack, upsert_shaderpack,
};
use velgrinor::skin::{
    SkinVariant, get_active_cape, get_active_skin, get_avatar_url, get_body_url,
    get_profile as get_mc_profile, get_skin_url, hide_cape, reset_skin, set_cape, set_skin_url,
    upload_skin,
};
use velgrinor::store::{ContentKind, store_content, store_from_url};
use velgrinor::template::{
    ContentSource, Template, TemplateLoader, TemplateRuntime, delete_template,
    init_builtin_templates, list_templates, load_template, save_template,
};

#[derive(Parser, Debug)]
#[command(name = "velgrinor", version, about = "VelGrinor Minecraft launcher")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}

const DEFAULT_RELEASE_MANIFEST_ENDPOINT: &str =
    "https://github.com/Sqrilizz/VelGrinor/releases/latest/download/latest.json";
...
impl From<StoreContentType> for ContentType {
    fn from(t: StoreContentType) -> Self {
        match t {
            ...
            StoreContentType::Modpack => ContentType::ModPack,
...
fn run() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::Launch {
            ...
            let profile_data = load_profile(&paths, &profile)?;
            let mut diagnostics = diagnose_profile(&paths, &profile_data)?;
            if diagnostics
                .issues
                .iter()
                .any(|issue| issue.code == "legacy_symlink")
            {
                materialize_instance(&paths, &profile_data)?;
                diagnostics = diagnose_profile(&paths, &profile_data)?;
            }
            let errors = diagnostics
                ...
                .collect::<Vec<_>>();
            if !errors.is_empty() {
                bail!("launch blocked: {}", errors.join("; "));
            }
            ...
            if !warnings.is_empty() && !force {
                bail!(
                    "launch warnings: {}; rerun with --force",
                    warnings.join("; ")
                );
            }
            ...
            if prepare_only {
                ...
                println!("game args: {}", redact_game_args(&plan.game_args).join(" "));
            } else {
...
fn redact_game_args(args: &[String]) -> Vec<String> {
    let mut result = args.to_vec();
    for index in 0..result.len().saturating_sub(1) {
        if matches!(
            result[index].as_str(),
            "--accessToken" | "--clientToken" | "--xuid"
        ) {
            result[index + 1] = "<redacted>".to_string();
        }
    }
    result
}
...
fn handle_app_update_command(command: AppUpdateCommand) -> Result<()> {
    match command {
        AppUpdateCommand::Check {
            ...
            let endpoint =
                endpoint.unwrap_or_else(|| DEFAULT_RELEASE_MANIFEST_ENDPOINT.to_string());
            let target = match platform {
                Some(value) => value,
                None => updater_target().context(
                    "unsupported OS/arch for updater target; use --platform to override",
                )?,
            };

            let client = Client::builder()
                .user_agent(format!("VelGrinorCLI/{}", env!("CARGO_PKG_VERSION")))
                .build()?;
            ...
            let current_version = current.unwrap_or_else(|| env!("CARGO_PKG_VERSION").to_string());
    ...
    match command {
        PackCommand::Add {
            ...
            let changed = match kind {
                ...
                ContentKind::Mod | ContentKind::ModPack | ContentKind::Skin => false,
            };
            save_profile(paths, &profile_data)?;
            if changed {
                println!("updated profile {profile}");
        ...
        PackCommand::List { profile } => {
            ...
            let list = match kind {
                ...
                ContentKind::Mod | ContentKind::ModPack | ContentKind::Skin => Vec::new(),
            };
            if list.is_empty() {
    ...
    match command {
        AccountCommand::Add {
            ...
            let client_id = client_id.or(config.msa_client_id).context(
                "missing Microsoft client id; set VELGRINOR_MS_CLIENT_ID or velgrinor config set-client-id",
            )?;
            let secret = client_secret.or(config.msa_client_secret);
            add_account_flow(paths, &client_id, secret.as_deref())?;
        }
        AccountCommand::Offline { username } => {
        ...
        StoreCommand::Install {
            ...
            if ct == ContentType::ModPack {
                let ver = if let Some(v) = version {
                    ...
                    store.get_latest_version(platform.into(), &project, None, None)?
                };
                let (archive, _) = store_from_url(paths, &ver.download_url)?;
                let imported = if matches!(platform, StorePlatform::Curseforge) {
                    let api_key = config
                        .curseforge_api_key
                        .as_deref()
                        .context("CurseForge API key is required")?;
                    import_curseforge_zip(paths, &archive, Some(&profile), api_key, |_, _| {})?
                } else {
                    import_mrpack(paths, &archive, Some(&profile))?
                };
                println!("installed {} as profile {}", item.name, imported.id);
                return Ok(());
            }
            ...
            let effective_loader: Option<String> = match ct {
                ContentType::Mod => profile_data.loader.as_ref().map(|l| l.loader_type.clone()),
            ...
            let versions = if ct == ContentType::Mod {
                let installed = profile_data
                    ...
                    .collect();
                let resolved = resolve_store_install_plan(
                    &store,
                    platform.into(),
                    ver,
                    &profile_data.mc_version,
                    effective_loader.as_deref(),
                    &installed,
                )?;
                if !resolved.plan.optional.is_empty() {
                    println!(
            ...
            let manager =
                DownloadManager::new(paths.downloads_state(), config.download_concurrency)?;
            let downloads = versions
                .iter()
                .map(|install_version| {
                    let destination = paths.cache_downloads.join(format!(
                        "store-{}-{}-{}",
                        platform.as_str(),
                        install_version.project_id,
                        install_version.id
                    ));
                    let mut request =
                        DownloadRequest::new(&install_version.download_url, &destination);
                    request.expected_size =
                        (install_version.size > 0).then_some(install_version.size);
                    request.sha1 = install_version.sha1.clone();
                    request.sha256 = install_version.sha256.clone();
                    request.group = Some(format!("store-install-{profile}"));
                    request.label = Some(install_version.filename.clone());
                    request
                })
                .collect();
            manager.run_requests(downloads, |_| {})?;
            let mut changed = false;
            for install_version in versions {
                let download = paths.cache_downloads.join(format!(
                    "store-{}-{}-{}",
                    platform.as_str(),
                    install_version.project_id,
                    install_version.id
                ...
                let content_ref = ContentRef {
                    ...
                    version: Some(install_version.version),
                    source: Some(install_version.download_url),
                    file_name: Some(stored.file_name),
                    platform: Some(platform.as_str().to_string()),
                    project_id: Some(install_version.project_id),
                    version_id: Some(install_version.id),
                    enabled: true,
                    pinned: false,
                };
                changed |= match ct {
                    ContentType::Mod => upsert_mod(&mut profile_data, content_ref),
                    ContentType::ModPack => unreachable!(),
                    ContentType::ResourcePack => {
                        upsert_resourcepack(&mut profile_data, content_ref)
                    }
    ...
    match command {
        ...
        LogsCommand::Show {
            ...
            let entries: Vec<_> = if let Some(query) = search {
                search_logs(&entries, &query).into_iter().cloned().collect()
            } else {
                entries
            };
...
fn create_profile_from_template(
    ...
    for mod_content in &template.mods {
        ...
        match &mod_content.source {
            ...
            ContentSource::Url { url } => match resolve_input(paths, url) {
            ...
            ContentSource::CurseForge { .. } => {
                println!("  ! {} (CurseForge requires API key)", mod_content.name);
            }
        }
    }
...
fn handle_library_command(paths: &Paths, command: LibraryCommand) -> Result<()> {
    ...
    match command {
                    ...
                    if delete_file {
                        ...
                        let store_path = match item.content_type {
                            ...
                            LibraryContentType::ShaderPack => {
                                paths.store_shaderpack_path(&item.hash)
                            }
        ...
        LibraryCommand::Stats => {
            ...
            println!("  Modpacks: {}", stats.modpacks_count);
            ...
            for profile_id in profiles {
                if let Ok(profile) = load_profile(paths, &profile_id) {
                    for content in profile
                        .mods
                        .iter()
                        .chain(profile.resourcepacks.iter())
                        .chain(profile.shaderpacks.iter())
                    {
                        if library
...
fn handle_modpack_command(paths: &Paths, command: ModpackCommand) -> Result<()> {
    match command {
        ModpackCommand::Import { path, id } => {
            let profile = if is_curseforge_zip(&path) {
                let config = load_config(paths)?;
                let api_key = config
                    .curseforge_api_key
                    .as_deref()
                    .context("CurseForge API key is required")?;
                import_curseforge_zip(paths, &path, id.as_deref(), api_key, |_, _| {})?
            } else {
                import_mrpack(paths, &path, id.as_deref())?
            };
