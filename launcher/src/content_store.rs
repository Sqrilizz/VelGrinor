...
impl ContentType {
    ...
    pub fn to_content_kind(self) -> crate::store::ContentKind {
        match self {
            ...
            ContentType::ModPack => crate::store::ContentKind::ModPack,
        }
    }
}
...
impl ContentStore {
    ...
    pub fn search(&self, options: &SearchOptions) -> Result<Vec<ContentItem>> {
        ...
        // Sort by downloads
        results.sort_by_key(|item| std::cmp::Reverse(item.downloads));
    ...
    pub fn search_modrinth(&self, options: &SearchOptions) -> Result<Vec<ContentItem>> {
        ...
        let result = self
            .modrinth
            .search(&options.query, &facets, limit, options.offset)?;
