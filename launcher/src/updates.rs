...
use crate::profile::{ContentRef, Profile, list_profiles, load_profile, save_profile};
...
pub fn get_storage_stats(paths: &Paths) -> Result<StorageStats> {
    let mut stats = StorageStats {
        mods_bytes: dir_size(&paths.store_mods)?,
        resourcepacks_bytes: dir_size(&paths.store_resourcepacks)?,
        shaderpacks_bytes: dir_size(&paths.store_shaderpacks)?,
        skins_bytes: dir_size(&paths.store_skins)?,
        ..StorageStats::default()
    };

    // Calculate Minecraft data size
    let minecraft_root = paths
        .minecraft_versions
        .parent()
        .unwrap_or(&paths.minecraft_versions);
    ...
    for (store_path, _) in [
        ...
        if store_path.exists()
            && let Ok(entries) = fs::read_dir(store_path)
        {
    ...
    for profile_id in profile_ids {
        let profile = match load_profile(paths, &profile_id) {
            ...
            Err(e) => {
                result
                    .errors
                    .push(format!("Failed to load profile {}: {}", profile_id, e));
                continue;
            }
        };
...
fn check_content_updates(
    ...
    for content in content_list {
        ...
        let latest =
            match store.get_latest_version(platform, project_id, Some(&profile.mc_version), loader)
            {
                Ok(v) => v,
                Err(e) => {
