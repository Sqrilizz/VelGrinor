...
use crate::util::{atomic_write, normalize_path_separator};
...
pub fn prepare(paths: &Paths, profile: &Profile, account: &LaunchAccount) -> Result<LaunchPlan> {
    prepare_with_progress(paths, profile, account, &mut |_, _| {})
}
...
pub fn prepare_with_progress(
    ...
    prepare_with_download_manager(paths, profile, account, &manager, progress, |_| {})
}

pub fn prepare_with_download_manager(
    paths: &Paths,
    profile: &Profile,
    account: &LaunchAccount,
    ...
    progress(2, "Linking profile content".to_string());
    let instance_dir = materialize_instance(paths, profile)?;

    progress(5, format!("Resolving Minecraft {}", profile.mc_version));
    let java_path = profile.runtime.java.as_deref();
    let version_id = resolve_version_id_managed(
        paths,
        &profile.mc_version,
        profile.loader.as_ref(),
        java_path,
        download_manager,
    )?;
    let resolved = resolve_version_managed(paths, &version_id, download_manager)?;
    ...
    let is_forge_loader = profile
        .loader
        .as_ref()
        .is_some_and(|l| l.loader_type == "forge" || l.loader_type == "neoforge");

    let mut client_jars = Vec::new();
    let client_count = resolved
        .chain
        .iter()
        .filter(|entry| entry.downloads.is_some())
        .count();
    let mut client_index = 0;
    for entry in &resolved.chain {
        if entry.downloads.is_some() {
            client_index += 1;
            let percent = 8 + scaled_progress(client_index, client_count, 4);
            progress(
                percent,
                format!("Game client {client_index}/{client_count}: {}", entry.id),
            );
            let jar_path = ensure_client_jar(paths, entry, download_manager, &download_progress)?;
    ...
    let asset_index_id = ensure_assets(
        paths,
        &version,
        download_manager,
        progress,
    ...
    let (classpath, natives_dir) = ensure_libraries(
        paths,
        &version,
        &instance_dir,
        &client_jars,
    ...
    progress(97, "Building launch arguments".to_string());
    ...
    progress(100, "Game files ready".to_string());
    Ok(LaunchPlan {
        instance_dir,
        java_exec,
        jvm_args,
...
pub fn launch(paths: &Paths, profile: &Profile, account: &LaunchAccount) -> Result<()> {
    ...
    configure_gpu_environment(&mut command);
    let status = command.status().context("failed to launch java")?;
...
pub fn configure_gpu_environment(command: &mut Command) -> bool {
    #[cfg(target_os = "linux")]
    {
...
fn resolve_version_id_managed(
    ...
    match loader {
        ...
        Some(loader) => match loader.loader_type.as_str() {
            "fabric" => ensure_fabric_profile(paths, mc_version, &loader.version, manager),
            "quilt" => ensure_quilt_profile(paths, mc_version, &loader.version, manager),
            "neoforge" => {
                ensure_neoforge_profile(paths, mc_version, &loader.version, java, manager)
            }
            "forge" => ensure_forge_profile(paths, mc_version, &loader.version, java, manager),
...
fn resolve_fabric_latest_version(paths: &Paths, manager: &DownloadManager) -> Result<String> {
    let url = "https://meta.fabricmc.net/v2/versions/loader";
    let json = download_json(paths, manager, url)?;
    let versions = json
        .as_array()
        .context("fabric loader versions not an array")?;
...
fn ensure_fabric_profile(
    ...
    let resolved_version = if loader_version.eq_ignore_ascii_case("latest") {
        resolve_fabric_latest_version(paths, manager)?
    } else {
    ...
    let profile_json = download_json(paths, manager, &url)?;
    let id = profile_json
        .get("id")
        .and_then(|v| v.as_str())
    ...
    if !target.exists() {
        ...
        atomic_write(&target, serde_json::to_vec_pretty(&profile_json)?).with_context(|| {
...
fn resolve_quilt_latest_version(paths: &Paths, manager: &DownloadManager) -> Result<String> {
    let url = "https://meta.quiltmc.org/v3/versions/loader";
    let json = download_json(paths, manager, url)?;
    let versions = json
        .as_array()
        .context("quilt loader versions not an array")?;
...
fn ensure_quilt_profile(
    ...
    // Resolve "latest" to actual version number
    let resolved_version = if loader_version.eq_ignore_ascii_case("latest") {
        resolve_quilt_latest_version(paths, manager)?
    } else {
        loader_version.to_string()
    };
    ...
    if !target.exists() {
        ...
        atomic_write(&target, serde_json::to_vec_pretty(&profile_json)?)
            .with_context(|| format!("failed to write quilt version json: {}", target.display()))?;
    }
    Ok(id.to_string())
}

/// Fetch the latest NeoForge version for a given Minecraft version
fn resolve_neoforge_latest_version(
    paths: &Paths,
    mc_version: &str,
    manager: &DownloadManager,
) -> Result<String> {
...
fn ensure_neoforge_profile(
    ...
    // Resolve "latest" to actual version number
    let resolved_version = if loader_version.eq_ignore_ascii_case("latest") {
        resolve_neoforge_latest_version(paths, mc_version, manager)?
    } else {
        loader_version.to_string()
    };
    ...
    let installer_path = paths
        .cache_downloads
        .join(format!("neoforge-{resolved_version}-installer.jar"));
    download_with_sha1(manager, &installer_url, &installer_path, None, &|_| {})?;
...
fn resolve_forge_latest_version(
    paths: &Paths,
    mc_version: &str,
    manager: &DownloadManager,
) -> Result<String> {
...
fn ensure_forge_profile(
    ...
    let resolved_loader = if loader_version.eq_ignore_ascii_case("latest") {
        resolve_forge_latest_version(paths, mc_version, manager)?
    } else {
        loader_version.to_string()
    };
    ...
    let profile_json = fs::read_to_string(&installer_json_path).with_context(|| {
        format!(
            "installer did not create expected version: {}",
            installer_id
        )
    })?;
    ...
    atomic_write(&target, serde_json::to_vec_pretty(&profile)?)
        .with_context(|| format!("failed to write forge version json: {}", target.display()))?;
...
fn run_forge_installer(
    ...
    eprintln!("Running installer to process libraries (this may take a minute)...");
...
fn resolve_version_managed(
    ...
    let mut current = load_version_json(paths, id, manager)?;
    loop {
        ...
        if let Some(parent) = current.inherits_from.clone() {
            current = load_version_json(paths, &parent, manager)?;
        } else {
            break;
        }
    }
...
fn load_version_json(paths: &Paths, id: &str, manager: &DownloadManager) -> Result<VersionJson> {
    ...
    let manifest = load_version_manifest(paths, manager)?;
    let entry = manifest
        .versions
        .iter()
        .find(|v| v.id == id)
    ...
    download_with_sha1(manager, &entry.url, &path, None, &|_| {})?;
    let data = fs::read_to_string(&path)
        .with_context(|| format!("failed to read version json: {}", path.display()))?;
...
fn load_version_manifest(paths: &Paths, manager: &DownloadManager) -> Result<VersionManifest> {
    ...
    download_with_sha1(manager, VERSION_MANIFEST_URL, &cache_path, None, &|_| {})?;
    let data = fs::read_to_string(&cache_path).with_context(|| {
        format!(
            "failed to read version manifest cache: {}",
            cache_path.display()
...
fn ensure_client_jar(
    ...
    version: &VersionJson,
    manager: &DownloadManager,
    progress: &(impl Fn(DownloadSnapshot) + Send + Sync),
) -> Result<PathBuf> {
...
fn ensure_assets(
    ...
    progress(13, format!("Asset index: {}", asset_index.id));
    download_with_sha1(
        manager,
        &asset_index.url,
        &index_path,
    ...
    let object_count = index.objects.len();
    ...
    for (name, object) in index.objects {
        ...
        if valid_existing(&object_path, Some(&object.hash)) {
            continue;
        }
        ...
        let mut request = DownloadRequest::new(url, object_path);
        request.sha1 = Some(object.hash);
        request.label = Some(name);
        request.group = Some(format!("minecraft-assets-{}", asset_index.id));
        requests.push(request);
    }
    if !requests.is_empty() {
        manager.run_requests(requests, |snapshot| {
            download_progress(snapshot.clone());
        })?;
        progress(73, format!("Assets {object_count}/{object_count}"));
    }
...
fn ensure_libraries(
    ...
    for library in &version.libraries {
        ...
        library_index += 1;
        let percent = 74 + scaled_progress(library_index, library_count, 21);
        progress(
            percent,
            format!(
                "Libraries {library_index}/{library_count}: {}",
                library.name
            ),
        );
            ...
            .and_then(|downloads| downloads.artifact.as_ref())
        {
            ...
            download_with_sha1(manager, &url, &lib_path, None, download_progress)?;
            classpath.push(lib_path);
        }
            ...
            && let Some(classifier) = natives.get(&os_key())
        {
                ...
                maven_path_from_name_with_classifier(&library.name, &classifier)
            {
                ...
                download_with_sha1(manager, &url, &jar_path, None, download_progress)?;
                extract_natives(&jar_path, &natives_dir, library.extract.as_ref())?;
            }
        }
    }
...
fn scaled_progress(current: usize, total: usize, span: u8) -> u8 {
    if total == 0 {
        return span;
    }
    ((current.saturating_mul(span as usize)) / total).min(span as usize) as u8
}
...
fn build_var_map(
    ...
    vars.insert("launcher_name".into(), "velgrinor".to_string());
    vars.insert(
...
fn resolve_java(override_java: Option<&str>, mc_version: &str) -> String {
    ...
    for install in &installations {
        if let Some(major) = install.major
            && is_java_compatible(major, mc_version)
        {
            eprintln!(
...
fn download_json(paths: &Paths, manager: &DownloadManager, url: &str) -> Result<Value> {
    let key = hex::encode(Sha1::digest(url.as_bytes()));
    let path = paths.cache_manifests.join(format!("remote-{key}.json"));
    download_with_sha1(manager, url, &path, None, &|_| {})?;
    let data = fs::read(&path).context("failed to read downloaded json")?;
    serde_json::from_slice(&data).context("failed to parse json")
}

fn valid_existing(path: &Path, expected_sha1: Option<&str>) -> bool {
    if let Some(expected) = expected_sha1 {
        return sha1_file(path).is_ok_and(|actual| actual.eq_ignore_ascii_case(expected));
    }
    path.metadata().is_ok_and(|metadata| metadata.len() > 0)
}

fn download_with_sha1(
    manager: &DownloadManager,
    url: &str,
    path: &Path,
    ...
    if valid_existing(path, expected_sha1) {
        return Ok(());
    }
    let mut request = DownloadRequest::new(url, path);
    request.sha1 = expected_sha1.map(str::to_string);
    request.label = path
        .file_name()
        .map(|name| name.to_string_lossy().into_owned());
    manager.run_requests(vec![request], progress)?;
    Ok(())
}
...
fn merge_versions(mut parent: VersionJson, mut child: VersionJson) -> VersionJson {
    ...
    if !parent.libraries.is_empty() {
        ...
        for lib in parent.libraries.iter() {
            let dominated = library_key(&lib.name).is_some_and(|key| seen.contains(&key));
            if !dominated {
                if let Some(key) = library_key(&lib.name) {
                    seen.insert(key);
                }
