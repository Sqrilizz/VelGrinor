#!/bin/bash
# Capture screenshots of the VelGrinor desktop app for the website
...
#
# This script captures the VelGrinor window WITH your actual macOS wallpaper,
# preserving the native liquid glass/vibrancy effect in the sidebar.
#
# IMPORTANT: You must be on the same desktop space as VelGrinor when running this.
...
# Get the window ID of the VelGrinor app (works across ALL spaces, not just current)
get_velgrinor_window_id() {
...
for window in windowList {
    if let owner = window[kCGWindowOwnerName as String] as? String,
       (owner == "velgrinor-launcher" || owner == "VelGrinor Launcher" || owner == "velgrinor_ui"),
...
// Second pass: fallback to window named "VelGrinor" if owned by velgrinor app
for window in windowList {
    if let name = window[kCGWindowName as String] as? String,
       let owner = window[kCGWindowOwnerName as String] as? String,
       name == "VelGrinor",
       owner.lowercased().contains("velgrinor"),
       let windowID = window[kCGWindowNumber as String] as? Int,
       let layer = window[kCGWindowLayer as String] as? Int,
       layer == 0 {
        print(windowID)
...
# Capture the VelGrinor window with wallpaper (region capture)
...
" 2>/dev/null)
    ...
    if screencapture -x -R "$bounds" "$output_file" 2>/dev/null; then
        ...
        if [ -f "$output_file" ] && [ "$file_size" -gt 10000 ]; then
            ...
            log_error "Capture failed - is the VelGrinor window visible on your current desktop?"
            log_info "Switch to the desktop where VelGrinor is and try again."
            return 1
        fi
    else
...
# List all VelGrinor-related windows (for debugging)
list_velgrinor_windows() {
    log_info "Searching for VelGrinor windows on all spaces..."
    swift -e '
import Cocoa
...
for window in windowList {
    ...
    if owner.lowercased().contains("velgrinor") || name.lowercased().contains("velgrinor") {
...
if !found {
    print("  No VelGrinor windows found")
    print("")
    print("  Make sure the VelGrinor app is running.")
...
main() {
    ...
    echo ""
    log_info "VelGrinor Screenshot Capture"
    log_info "=================================="
    echo ""
    ...
    # Find the VelGrinor window
    log_step "Finding VelGrinor window..."
    local window_id
    window_id=$(get_velgrinor_window_id)

    if [ -z "$window_id" ]; then
        log_error "Could not find VelGrinor window!"
        echo ""
        list_velgrinor_windows
        echo ""
        log_info "Tips:"
        log_info "  1. Make sure VelGrinor is running (cargo tauri dev)"
    ...
    capture_and_convert() {
        local name="$1"
        local temp_png="/tmp/velgrinor_capture_${name}.png"
    ...
    if [ "$page" != "all" ] && [ "$page" != "list" ]; then
        ...
        log_info "  1. Switch to the desktop where VelGrinor is visible"
        log_info "  2. Navigate to the page in VelGrinor"
...
# Show help
if [ "$1" = "-h" ] || [ "$1" = "--help" ]; then
    echo "VelGrinor Screenshot Capture"
    echo ""
    ...
    echo "  list       - List all VelGrinor windows (debugging)"
    echo ""
    echo "Environment:"
    echo "  MARGIN=32  - Wallpaper margin around window (default: 32)"
    echo ""
    echo "Workflow:"
    echo "  1. Switch to the desktop where VelGrinor is (e.g., Ctrl+Right)"
    echo "  2. Make sure VelGrinor window is visible (not covered by other windows)"
    echo "  3. Run: $0 overview"
    echo "  4. Navigate to Library in VelGrinor, run: $0 library"
