...
export function LogsView() {
  ...
  const loadFile = useCallback(async (file: LogFile) => {
    ...
    try {
      ...
      notify(t("Failed to load file"), String(err));
    } finally {
      setLoading(false);
    }
  ...
  const analyzeLatestCrash = async () => {
    if (!selectedProfileId) return;
    try {
      setCrashAnalysis(await invoke<CrashAnalysis>("analyze_last_crash_cmd", {
        profileId: selectedProfileId,
        exitCode: launchStatus?.exit ?? null,
      }));
    } catch (err) {
      notify(t("Crash analysis failed"), String(err));
    }
  };

  const disableSuspectedMod = async () => {
    if (!selectedProfileId || !crashAnalysis?.suspected_mod) return;
    const suspectedMod = crashAnalysis.suspected_mod;
    if (!window.confirm(t("Disable suspected mod {{mod}}? A profile snapshot will be created first.", { mod: suspectedMod }))) return;
    try {
      await invoke("disable_suspected_mod_cmd", { profileId: selectedProfileId, suspectedMod });
      await useAppStore.getState().loadProfile(selectedProfileId);
      notify(t("Suspected mod disabled"), suspectedMod);
      await analyzeLatestCrash();
    } catch (err) {
      notify(t("Failed to disable suspected mod"), String(err));
    }
  };
    ...
    return (
      <div className="view-transition" >
        <div className="logs-empty-state">
          ...
          <p>{t("Select a profile to view logs")}</p>
        </div>
      </div>
    );
  }
      ...
      <div className="logs-header">
        ...
        {(tab === "latest" || selectedFile) && (
          <div className="logs-controls">
            ...
            >
              <option value="debug">{t("All")}</option>
        ...
        {tab === "crashes" && (
          <button className="btn btn-secondary btn-sm" onClick={() => void analyzeLatestCrash()}>{t("Analyze latest crash")}
          </button>
        )}
      </div>

      {tab === "crashes" && crashAnalysis && (
        <div className="crash-analysis-panel">
          <div className="crash-analysis-heading"><span>{t(crashAnalysis.category)}</span><strong>{t(crashAnalysis.probable_cause)}</strong></div>
          {crashAnalysis.evidence.length > 0 && <pre>{crashAnalysis.evidence.join("\n")}</pre>}
          <div className="crash-analysis-actions">{crashAnalysis.actions.map((action) => <span key={action}>{t(action)}</span>)}</div>
          {crashAnalysis.suspected_mod && (
            <button className="btn btn-danger btn-sm" onClick={() => void disableSuspectedMod()}>
              {t("Disable suspected mod {{mod}}", { mod: crashAnalysis.suspected_mod })}
            </button>
          )}
        </div>
      )}
            ...
            <div className="empty-state-container">
              <div className="empty-state">
                ...
                <p style={{ marginBottom: 0 }}>
                  {tab === "crashes"
                    ? t("Crash reports appear when the game encounters errors")
                    : t("Launch the game to generate logs")}
                </p>
              </div>
            </div>
          ) : (
              ...
              >
                ...
                <div className="logs-file-info">
                  ...
                  <span className="logs-file-meta">
                    ...
                    {file.is_current && <span className="logs-file-current">{t("Current")}</span>}
                  </span>
                </div>
          ...
          {!loading && filteredLogs.length === 0 && (
            <div className="empty-state-container" style={{ flex: 1 }}>
              <div className="empty-state">
                ...
                <h3>{logs.length === 0 ? t("No logs yet") : t("No matching logs")}</h3>
                <p style={{ marginBottom: 0 }}>
                  {logs.length === 0
                    ? t("Launch the game to see live output")
                    : t("Try adjusting your filter settings")}
                </p>
              </div>
            </div>
          )}
          ...
          {!loading && filteredLogs.length > 0 && (
            <>
              ...
              <div className="logs-status">
                <span>{filteredLogs.length}{t("of")} {logs.length}{t("entries")}</span>
                {tab === "latest" && isGameRunning && (
                  <span className="logs-status-live">
                    <span className="logs-live-dot" />{t("Live")}
                  </span>
                )}
              </div>
            </>
