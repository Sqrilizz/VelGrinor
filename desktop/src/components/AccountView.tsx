...
export function AccountView({ onAddAccount }: AccountViewProps) {
  ...
  const hasLoadedSkinLibraryRef = useRef(false);
  ...
  useEffect(() => {
    ...
    invoke<Account>("refresh_account_session_cmd", { id: activeAccount.uuid })
      .then(async () => {
        if (cancelled) return;
        setSessionError(null);
        await loadAccounts();
  ...
  }, [activeAccount?.uuid, activeAccount?.kind, loadAccounts]);
  ...
  useEffect(() => {
    const delay = hasLoadedSkinLibraryRef.current ? 300 : 0;
    hasLoadedSkinLibraryRef.current = true;
  ...
  const removeFromLibrary = useCallback(async (item: SkinLibraryItemWithUrl) => {
    try {
      ...
      notify(t("Skin removed from library"));
    } catch (err) {
      notify(t("Failed to remove skin"), String(err));
    }
  ...
  const handleRemoveAccount = async (account: Account) => {
    if (!confirm(t("Remove {{name}}? This account will be disconnected from VelGrinor.", { name: account.username }))) {
      return;
    }
    await runAction(async () => {
  ...
  const handleUploadSkin = async () => {
    ...
    try {
      ...
      notify(t("Skin uploaded and added to library"));
    } catch (err) {
      setError(String(err));
    } finally {
      setUploading(false);
  ...
  const handleApplyLibrarySkin = async (item: SkinLibraryItemWithUrl) => {
    ...
    try {
      ...
      notify(t("Skin applied successfully"));
    } catch (err) {
      setError(String(err));
    } finally {
      setUploading(false);
  ...
  const handleResetSkin = async () => {
    ...
    try {
      ...
      await loadAccountInfo(activeAccount.uuid);
      notify(t("Skin reset to default"));
    } catch (err) {
      setError(String(err));
    } finally {
      setUploading(false);
    ...
    return (
      <div className="view-transition" >
        ...
        <div className="account-empty-state">
          ...
          <h3>{t("No accounts connected")}</h3>
          <p>{t("Add an offline or Microsoft account to play Minecraft.")}</p>
          <button className="btn btn-primary" onClick={onAddAccount}>{t("Add Account")}
          </button>
        </div>
      </div>
    );
    ...
    <div className="view-transition account-view" >
      ...
      {error && !loading && (
        <div className="account-error">
          <p>{error}</p>
          <button className="btn btn-secondary btn-sm" onClick={() => activeAccount && loadAccountInfo(activeAccount.uuid)}>{t("Retry")}
          </button>
        </div>
      )}

      {activeAccount?.kind === "offline" && (
        <div className="account-session-banner account-session-banner-warning">
          <div>
            <strong>{t("Offline account")}</strong>
            <span>{t("Online-mode servers and Microsoft services are unavailable.")}</span>
          </div>
        </div>
      )}

      {activeAccount?.kind === "microsoft" && sessionError && (
        <div className="account-session-banner account-session-banner-danger">
          <div>
            <strong>{t("Microsoft session expired")}</strong>
            <span>{t("Sign in again to continue using this account.")}</span>
          </div>
          <button className="btn btn-primary btn-sm" onClick={onAddAccount}>{t("Sign in again")}</button>
        </div>
      )}

      {activeAccount?.kind === "microsoft" && refreshingSession && (
        <div className="account-session-state">{t("Checking Microsoft session...")}</div>
      )}
                  ...
                  {activeAccount && (
                    <>
                      ...
                      <div className="account-selector-info">
                        ...
                        <span className="account-selector-hint">{t("Click to switch accounts")}</span>
                      </div>
                    ...
                    {accounts.accounts.map((account) => (
                      ...
                      >
                        ...
                        <div className="account-selector-option-info">
                          ...
                          <span className="account-selector-option-uuid">{account.kind === "offline" ? "Offline" : "Microsoft"} · {account.uuid.slice(0, 8)}...</span>
                        </div>
                ...
                <div className="account-skin-tab">
                  ...
                  <div className="account-upload-section">
                    ...
                    >
                      ...
                      {uploading ? t("Uploading...") : "Upload Skin File"}
                    </button>
                    <p className="account-upload-hint">{t("64x64 or 64x32 PNG file")}</p>
                  </div>
                ...
                <div className="account-library-tab">
                  ...
                  {libraryLoading ? (
                    ...
                    <div className="account-library-empty">
                      ...
                      <p>{t("No skins in library")}</p>
                      <p className="hint">{t("Upload skins to build your collection for quick switching.")}</p>
                    </div>
                  ) : (
                ...
                <div className="account-capes-tab">
                  {(info.profile?.capes?.length ?? 0) === 0 ? (
                    <div className="account-capes-empty">
                      <p>{t("No capes available")}</p>
                      <p className="hint">{t("Capes are obtained through Minecraft events, purchases, or promotions.")}</p>
                    </div>
                  ) : (
                      ...
                      {activeCape && (
                        ...
                        >
                          ...
                          <div className="account-cape-info">
                            <span className="account-cape-name">{t("No Cape")}</span>
                            <span className="account-cape-hint">{t("Hide your cape")}</span>
                          </div>
                        </button>
                      )}
                      {(info.profile?.capes ?? []).map((cape: Cape) => (
                        ...
                        >
                          ...
                          <div className="account-cape-info">
                            ...
                            {cape.state === "ACTIVE" && (
                              <span className="account-cape-active">{t("Currently equipped")}</span>
                            )}
                          </div>
                          {cape.state !== "ACTIVE" && (
                            <span className="account-cape-equip">{t("Equip")}</span>
                          )}
                        </button>
                      ))}
                    </div>
