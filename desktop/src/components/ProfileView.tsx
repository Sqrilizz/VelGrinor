...
import { getModrinthProjectId, useProjectIcons } from "../hooks/useProjectIcons";
...
type DiagnosticReport = {
  blocking: boolean;
  issues: Array<{ code: string; severity: "warning" | "error"; message: string; evidence: string; fix?: Record<string, unknown> | null }>;
};
...
export function ProfileView({
  ...
  const allContent = profile
    ? [...profile.mods, ...profile.resourcepacks, ...profile.shaderpacks]
    : [];
  const projectIcons = useProjectIcons(allContent.map(getModrinthProjectId));
  ...
  const handleVersionSelect = async (version: string) => {
    ...
    try {
      ...
      notify(t("Version updated"), `Changed to ${version}`);
    } catch (err) {
      notify(t("Failed to update version"), String(err));
    } finally {
      setSaving(false);
      setExpandedDropdown(null);
    }
  ...
  const handleLoaderSave = async () => {
    ...
    try {
      ...
      await loadProfile(profile.id);
      notify(t("Loader updated"), selectedLoaderType ? `Changed to ${selectedLoaderType}` : "Set to Vanilla");
    } catch (err) {
      notify(t("Failed to update loader"), String(err));
    } finally {
      setSaving(false);
      setExpandedDropdown(null);
    }
  ...
  const handleTogglePin = async (item: ContentRef) => {
    ...
    try {
      ...
      notify(t("Failed to update pin"), String(err));
    }
    setTogglingPin(null);
  };
  ...
  const handleToggleEnabled = async (item: ContentRef) => {
    ...
    try {
      ...
      notify(t("Failed to update enabled state"), String(err));
    }
  ...
  const handleRepair = async () => {
    ...
    setMaintaining(true);
    try {
      const report = await invoke<{ checked: number; repaired: number; missing: string[]; corrupt: string[]; errors: string[]; conflicts: string[] }>("repair_profile_cmd", { profileId: profile.id });
      notify(
        "Profile checked",
        report.errors.length > 0 || report.conflicts.length > 0
          ? `Checked ${report.checked}, repaired ${report.repaired}, issues ${report.errors.length + report.conflicts.length}`
          : `Checked ${report.checked} files, repaired ${report.repaired}`
      );
    } catch (err) {
      notify(t("Profile repair failed"), String(err));
    } finally {
  ...
  const handleDiagnostics = async () => {
    ...
    try {
      const [report, history] = await Promise.all([
        invoke<DiagnosticReport>("diagnose_profile_cmd", { profileId: profile.id }),
        invoke<SnapshotInfo[]>("list_snapshots_cmd", { profileId: profile.id }),
      ]);
  ...
  const handleDiagnosticFix = async (fix: Record<string, unknown>) => {
    if (!profile) return;
    setMaintaining(true);
    try {
      const report = await invoke<DiagnosticReport>("apply_diagnostic_fix_cmd", { profileId: profile.id, fix });
      setDiagnostics(report);
      setSnapshots(await invoke<SnapshotInfo[]>("list_snapshots_cmd", { profileId: profile.id }));
      await loadProfile(profile.id);
    } catch (err) {
      notify(t("Fix failed"), String(err));
    } finally {
      setMaintaining(false);
    }
  };
  ...
  const handleCreateSnapshot = async () => {
    if (!profile) return;
    await invoke("create_snapshot_cmd", { profileId: profile.id, reason: "manual snapshot" });
    setSnapshots(await invoke<SnapshotInfo[]>("list_snapshots_cmd", { profileId: profile.id }));
    notify(t("Snapshot created"));
  };

  const handleRollback = async (snapshotId: string) => {
    if (!profile || !window.confirm(t("Restore this snapshot? Current profile settings will be saved first."))) return;
    await invoke("rollback_snapshot_cmd", { profileId: profile.id, snapshotId });
    await loadProfile(profile.id);
  ...
  const handleExport = async () => {
    ...
    const outputPath = await save({
      defaultPath: `${profile.id}.mrpack`,
      filters: [{ name: "Modrinth Modpack", extensions: ["mrpack"] }],
    });
    if (!outputPath) return;
    setMaintaining(true);
    try {
      await invoke("export_profile_mrpack_cmd", { profileId: profile.id, outputPath });
      notify(t("Modpack exported"), outputPath);
    } catch (err) {
      notify(t("Export failed"), String(err));
    } finally {
      setMaintaining(false);
    }
  };
  ...
  const handleUpdateAll = async () => {
    ...
    setMaintaining(true);
    try {
      const result = await invoke<{ updates: Array<{ profile_id: string; content: ContentRef; content_type: string; latest_version_id: string }> }>("check_profile_updates_cmd", { profileId: profile.id });
      const updates = result.updates.filter((update) => !update.content.pinned);
      if (updates.length > 0) {
        await invoke("backup_profile_cmd", { profileId: profile.id });
      }
      ...
      await loadProfile(profile.id);
      notify(updates.length ? "Profile updated" : "All up to date", updates.length ? `Updated ${updates.length} items` : "No updates available");
    } catch (err) {
      notify(t("Update failed"), String(err));
    } finally {
      setMaintaining(false);
    }
  };
    ...
    <div className="view-transition" >
      ...
      <div className="profile-header">
          ...
          <div className="profile-chips" ref={dropdownRef}>
            ...
            <div className="chip-dropdown-wrapper">
              <button
                ...
                title={t("Change Minecraft version")}
              >
                {saving && expandedDropdown === "version" ? t("Saving...") : profile.mcVersion}
              ...
              {expandedDropdown === "version" && (
                <div className="chip-dropdown">
                  ...
                  <div className="chip-dropdown-list">
                    {mcVersionsLoading ? (
                      <div className="chip-dropdown-loading">{t("Loading versions...")}</div>
                    ) : (
            ...
            <div className="chip-dropdown-wrapper">
              <button
                ...
                title={t("Change mod loader")}
              >
                {saving && expandedDropdown === "loader" ? t("Saving...") : loaderLabel}
              ...
              {expandedDropdown === "loader" && (
                <div className="chip-dropdown">
                  ...
                  <div className="chip-dropdown-section">
                    ...
                    >
                      <option value="">{t("Vanilla (no loader)")}</option>
                      <option value="fabric">{t("Fabric")}</option>
                      <option value="quilt">{t("Quilt")}</option>
                      <option value="forge">{t("Forge")}</option>
                      <option value="neoforge">{t("NeoForge")}</option>
                    </select>
                  </div>
                  {selectedLoaderType && (
                    <div className="chip-dropdown-section">
                      <label className="chip-dropdown-label">
                        {selectedLoaderType.charAt(0).toUpperCase() + selectedLoaderType.slice(1)}{t("Version")}
                      </label>
                      <select
                        className="select select-sm"
                      ...
                      >
                        {loaderVersionsLoading ? (
                          ...
                          <option value="">{t("No versions available")}</option>
                        ) : (
                          <>
                            {!selectedLoaderVersion && <option value="">{t("Select version...")}</option>}
                            ...
                            {selectedLoaderVersion && !loaderVersions.includes(selectedLoaderVersion) && (
                              <option key={selectedLoaderVersion} value={selectedLoaderVersion}>
                                {selectedLoaderVersion}{t("(current)")}
                              </option>
                            )}
                  ...
                  <div className="chip-dropdown-footer">
                    <button className="btn btn-ghost btn-sm" onClick={() => setExpandedDropdown(null)}>{t("Cancel")}
                    </button>
                    <button
        ...
        >
          ...
          <span>{launchStatus ? `${t(launchStatus.stage.charAt(0).toUpperCase() + launchStatus.stage.slice(1))}${launchStatus.stage === "preparing" && launchStatus.progress != null ? ` ${launchStatus.progress}%` : ""}` : t("Launch")}</span>
        </button>
      </div>

      {reliabilityOpen && (
        <div className="section-panel reliability-panel">
          <div className="section-header">
            <div>
              <h2>{t("Diagnostics & snapshots")}</h2>
              <p>{diagnostics?.blocking ? t("Launch is blocked until errors are fixed") : diagnostics?.issues.length ? t("Warnings found; launch remains available") : t("Profile is ready to launch")}</p>
            </div>
            <div className="profile-maintenance-actions">
              <button className="btn btn-ghost btn-sm" onClick={handleCreateSnapshot}>{t("Create snapshot")}</button>
              <button className="btn btn-ghost btn-sm" onClick={() => setReliabilityOpen(false)}>{t("Close")}</button>
            </div>
          </div>
          <div className="reliability-grid">
            <div className="diagnostic-list">
              {diagnostics?.issues.length ? diagnostics.issues.map((issue, index) => (
                <div className={clsx("diagnostic-item", `is-${issue.severity}`)} key={`${issue.code}-${index}`}>
                  <div><strong>{issue.message}</strong><span>{issue.evidence}</span></div>
                  {issue.fix && <button className="btn btn-secondary btn-sm" disabled={maintaining} onClick={() => void handleDiagnosticFix(issue.fix!)}>{t("Fix")}</button>}
                </div>
              )) : <div className="diagnostic-empty">{t("No problems found")}</div>}
            </div>
            <div className="snapshot-list">
              {snapshots.map((snapshot) => (
                <div className="snapshot-item" key={snapshot.id}>
                  <div><strong>{new Date(snapshot.created_at * 1000).toLocaleString()}</strong><span>{snapshot.reason} · {(snapshot.size / 1024).toFixed(1)}{t("KB")} · {snapshot.automatic ? t("automatic") : t("manual")}</span></div>
                  <button className="btn btn-ghost btn-sm" onClick={() => void handleRollback(snapshot.id)}>{t("Restore")}</button>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="section-panel">
        <div className="content-tabs-row">
          <div className="content-tabs">
            <button className={clsx("content-tab", activeTab === "resourcepacks" && "active")} onClick={() => setActiveTab("resourcepacks")}>{t("Packs")}<span className="count">{contentCounts.resourcepacks}</span>
            </button>
            <div
              className={clsx("content-tab-wrapper", !hasModLoader && "disabled")}
              data-tooltip={hasModLoader ? undefined : t("Install a mod loader to enable shaders")}
            >
              <button
              ...
              >{t("Shaders")}<span className="count">{contentCounts.shaderpacks}</span>
              </button>
            </div>
            <div
              className={clsx("content-tab-wrapper", !hasModLoader && "disabled")}
              data-tooltip={hasModLoader ? undefined : t("Install a mod loader to enable mods")}
            >
              <button
                className={clsx("content-tab", activeTab === "mods" && "active", !hasModLoader && "disabled")}
                onClick={() => hasModLoader && setActiveTab("mods")}
              >{t("Mods")}<span className="count">{contentCounts.mods}</span>
              </button>
            </div>
          </div>
          <button
            ...
            title={t("Add {{type}}", { type: getContentTypeLabel(activeTab) })}
          >
        ...
        {contentItems.length === 0 ? (
          <div className="empty-state-inline">
            <span>{t("No")} {getContentTypeLabelPlural(activeTab)}{t("installed")}</span>
            <button className="link" onClick={() => onAddContent(activeTab)}>{t("+ Add")}</button>
          </div>
        ) : (
              ...
              return (
                <ContentItemRow
                  ...
                  item={{
                    ...
                    source: item.source,
                    icon_url: projectIcons[getModrinthProjectId(item) ?? ""] ?? null,
                  ...
                  actions={
                    <>
                      <button
                        ...
                        title={isEnabled ? t("Disable (won't load)") : t("Enable (load in instance)")}
                      >
                      ...
                      {platform !== "local" && (
                        <button
                          ...
                          title={isPinned ? t("Unpin (allow auto-updates)") : t("Pin (prevent auto-updates)")}
                        >
      ...
      <div className="section-panel">
        ...
        <div className="actions-row">
          <button className="btn btn-ghost btn-sm" onClick={onOpenInstance}>{t("Open folder")}</button>
          <button className="btn btn-ghost btn-sm" onClick={onCopyCommand}>{t("Copy CLI command")}</button>
          <button className="btn btn-ghost btn-sm" onClick={onShowJson}>{t("View JSON")}</button>
          <button className="btn btn-ghost btn-sm" onClick={handleUpdateAll} disabled={maintaining}>{t("Update all")}</button>
          <button className="btn btn-ghost btn-sm" onClick={handleDiagnostics} disabled={maintaining}>{t("Diagnostics")}</button>
          <button className="btn btn-ghost btn-sm" onClick={handleRepair} disabled={maintaining}>{t("Repair files")}</button>
          <button className="btn btn-ghost btn-sm" onClick={handleExport} disabled={maintaining}>{t("Export .mrpack")}</button>
        </div>
      </div>
    </div>
  );
