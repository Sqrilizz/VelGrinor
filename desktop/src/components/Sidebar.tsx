    ...
    >
      ...
      >
        ...
        {isEditing ? (
          ...
          <span className="profile-dropdown-folder-name">{folder.name || t("New Folder")}</span>
        )}
  ...
  const handleFinishProfileRename = async () => {
    ...
    if (newName && newName !== renamingProfileId) {
      try {
        ...
        notify(t("Rename failed"), String(err));
      }
    }
    ...
    <aside className="sidebar">
      ...
      <div className="profile-context" ref={profileMenuRef}>
        ...
        >
          ...
          {selectedProfileId ? (
            ...
            <div className="profile-context-info">
              <span className="profile-context-placeholder">{t("Select a profile")}</span>
            </div>
          )}
          ...
          <div className="profile-dropdown">
            ...
            >
              <div className="profile-dropdown-list">
                ...
                {profiles.length === 0 && (
                  <div className="profile-dropdown-empty">{t("No profiles yet")}</div>
                )}
              </div>
      ...
      <div className="sidebar-footer">
        {activeAccount ? (
          ...
          <button className="btn btn-secondary btn-sm w-full" onClick={onAddAccount} data-tauri-drag-region="false">{t("Add account")}
          </button>
        )}
      </div>
        ...
        >
          {contextMenuTarget.type === "profile" && (
            <>
              ...
              >
                {isFavorite(contextMenuTarget.id) ? t("Remove from favorites") : t("Set as favorite")}
              </button>
              {profileOrg.folders.length > 0 && (
                <>
                  <div className="menu-divider" />
                  <div className="menu-label">{t("Move to folder")}</div>
