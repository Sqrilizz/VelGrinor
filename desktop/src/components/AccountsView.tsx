...
export function AccountsView({ onSetActive, onRemove, onAdd, onViewDetails }: AccountsViewProps) {
  ...
  return (
    <div className="view-transition">
      ...
      <p style={{ margin: "-24px 0 24px", fontSize: 14, color: "var(--text-secondary)" }}>{t("Manage your Microsoft accounts for Minecraft. Click on an avatar to customize skin and cape.")}
      </p>
        ...
        <div key={account.uuid} className="setting-row">
          ...
          <div className="setting-control">
            {accounts.active === account.uuid ? (
              <span style={{ fontSize: 13, color: "rgba(255,255,255,0.5)" }}>{t("Active")}</span>
            ) : (
              <button className="btn btn-secondary btn-sm" onClick={() => onSetActive(account.uuid)}>{t("Use")}</button>
            )}
            <button className="btn btn-ghost btn-sm" onClick={() => onRemove(account)}>{t("Remove")}</button>
          </div>
        </div>
      ))}
      ...
      {(!accounts || accounts.accounts.length === 0) && (
        <div className="empty-state">
          <h3>{t("No accounts")}</h3>
          <p>{t("Add a Microsoft account to play Minecraft.")}</p>
        </div>
      )}

      <div style={{ marginTop: 24 }}>
        <button className="btn btn-primary" onClick={onAdd}>{t("Add account")}</button>
      </div>
    </div>
  );
}
