...
export function SettingsView() {
  const { notify, config, setConfig } = useAppStore();
  ...
  const loadStats = useCallback(async () => {
    try {
      ...
      notify(t("Failed to load storage stats"), String(err));
    }
  }, [notify]);
  ...
  const loadJavaInstallations = useCallback(async () => {
    ...
    try {
      ...
      notify(t("Failed to detect Java"), String(err));
    }
  ...
  const handleAutoUpdateToggle = async () => {
    ...
    try {
      ...
      notify(t("Settings saved"), `Auto-update ${newValue ? "enabled" : "disabled"}`);
    } catch (err) {
  ...
  const handleDiscordRpcToggle = async () => {
    const enabled = !discordRpc;
    try {
      const updated = await invoke<import("../types").Config>("set_discord_rpc_enabled_cmd", { enabled });
  ...
  const handleCheckUpdates = async () => {
    ...
    try {
      ...
      if (result.updates.length === 0) {
        notify(t("All up to date"), `Checked ${result.checked} items, no updates available`);
      } else {
        notify(t("Updates available"), `Found ${result.updates.length} updates`);
      }
    } catch (err) {
      notify(t("Update check failed"), String(err));
    }
  ...
  const handleApplyUpdate = async (update: ContentUpdate) => {
    ...
    try {
      ...
      notify(t("Update applied"), `${update.content.name} updated to ${update.latest_version}`);
  ...
  const handleCheckAppUpdate = async () => {
    ...
    try {
      ...
      setAppUpdate(update);
      if (update) {
        notify(t("App update available"), `Version ${update.version}`);
      } else {
        notify(t("VelGrinor is up to date"), `You're running ${appVersion ?? "the latest version"}`);
      }
    } catch (err) {
      notify(t("App update check failed"), String(err));
    }
  ...
  const handleInstallAppUpdate = async () => {
    ...
    try {
      ...
      notify(t("Update installed"), "Restart VelGrinor to finish updating");
      setAppUpdate(null);
    } catch (err) {
      notify(t("Update failed"), String(err));
    }
    ...
    <div className="view-transition settings-layout">
      ...
      <div className="settings-selector">
        ...
        <div className="settings-select-display">
          {activeItem?.icon}
          <span>{activeItem ? t(activeItem.label) : ""}</span>
        ...
        {activeSection === "general" && (
          <>
            <section className="settings-card" style={{ marginBottom: 24 }}>
              ...
              <div className="settings-row">
                <div className="settings-row-content">
                  <div className="settings-row-title">{t("Auto-check for updates")}</div>
                  <div className="settings-row-description">{t("Check for mod updates when the launcher starts")}</div>
                </div>
                <button
              ...
              <div className="settings-row">
                <div className="settings-row-content">
                  ...
                  <div className="settings-row-description">{t("English / Русский")}</div>
                </div>
              ...
              <div className="settings-row">
                <div className="settings-row-content">
                  <div className="settings-row-title">{t("Discord Rich Presence")}</div>
                  <div className="settings-row-description">{t("Show your launcher and Minecraft activity in Discord")}</div>
                </div>
                <button
                  className="toggle-switch"
                  data-active={discordRpc}
                  onClick={handleDiscordRpcToggle}
                  aria-label={t("Discord Rich Presence")}
                >
                  <span className="toggle-switch-thumb" />
                </button>
              </div>
            ...
            <section className="settings-card" style={{ marginBottom: 24 }}>
              ...
              <div className="settings-row">
                <div className="settings-row-content">
                  <div className="settings-row-title">{t("Open data folder")}</div>
                  <div className="settings-row-description">{t("View profiles, store, and configuration files")}</div>
                </div>
                <button className="btn btn-secondary btn-sm" onClick={handleOpenDataFolder}>{t("Open")}
                </button>
              </div>
            </section>
            ...
            <section className="settings-card settings-card-muted">
              ...
              <p className="settings-tip">{t("Pin content in the profile view to prevent auto-updates. Pinned mods, shaders, and resource packs stay at their current version.")}
              </p>
            </section>
          </>
        )}
        ...
        {activeSection === "storage" && stats && (
          <>
            <section className="settings-card" style={{ marginBottom: 24 }}>
                ...
                {categories.map((cat) => {
                  ...
                  return (
                    <div
                      ...
                      title={`${t(cat.label)}: ${formatFileSize(cat.bytes)}`}
                    />
                  );
                })}
              </div>
              ...
              <div className="storage-legend">
                {categories.map((cat) => (
                  <div key={cat.key} className="storage-legend-item">
                    ...
                    <span className="storage-legend-label">{t(cat.label)}</span>
              ...
              <div className="storage-stats">
                <div className="storage-stat">
                  ...
                  <span className="storage-stat-label">{t("files")}</span>
                </div>
                <div className="storage-stat">
                  <span className="storage-stat-value">{stats.total_references}</span>
                  <span className="storage-stat-label">{t("references")}</span>
                </div>
                {stats.deduplication_savings > 0 && (
                  <div className="storage-stat storage-stat-highlight">
                    ...
                    <span className="storage-stat-label">{t("saved")}</span>
                  </div>
                )}
              </div>
            </section>
            ...
            <section className="settings-card" style={{ marginBottom: 24 }}>
              ...
              <div className="settings-row">
                <div className="settings-row-content">
                  <div className="settings-row-title">{t("Clean unused content")}</div>
                  <div className="settings-row-description">{t("Remove items from the library that are not used by any profile")}</div>
                </div>
                <button className="btn btn-secondary btn-sm" onClick={() => setPurgeModalOpen(true)}>{t("Clean")}
                </button>
              </div>
            </section>
            ...
            <section className="settings-card settings-card-muted">
              <div className="settings-card-header">
                ...
                <span>{t("Deduplication")}</span>
                {stats.deduplication_savings > 0 && (
                  <span className="settings-card-badge" style={{ marginLeft: "auto", background: "rgba(52, 211, 153, 0.15)", color: "#34d399" }}>
                    {formatFileSize(stats.deduplication_savings)}{t("saved")}
                  </span>
                )}
              </div>
              <p className="settings-tip">{t("VelGrinor uses content-addressed storage with SHA-256 hashing. When the same mod is used across multiple profiles, it's stored only once.")}
                {stats.deduplication_savings > 0 && stats.total_references > stats.unique_items && (
                  <>{t("You have")} {stats.total_references}{t("references to")} {stats.unique_items}{t("unique files, saving")} <strong style={{ color: "#34d399" }}>{formatFileSize(stats.deduplication_savings)}</strong>{t("of disk space.")}</>
                )}
              </p>
            </section>
          </>
          ...
          <>
            <section className="settings-card" style={{ marginBottom: 24 }}>
              <div className="settings-card-header">
                ...
                <span>{t("Detected Installations")}</span>
                <button
              ...
              {detectingJava && javaInstallations.length === 0 ? (
                <p className="settings-muted">{t("Detecting Java installations...")}</p>
              ) : javaInstallations.length === 0 ? (
                <p className="settings-muted">{t("No Java installations found")}</p>
              ) : (
                <div className="java-list">
                  {javaInstallations.map((java, i) => (
                    <div key={i} className="java-item">
                      <div className="java-item-info">
                        <div className="java-item-version">{t("Java")} {java.major ?? "?"}
                      ...
                      <div className="java-item-badges">
                        ...
                        {java.is_valid ? (
                          ...
                          <span className="badge badge-warning">{t("Invalid")}</span>
                        )}
                      </div>
                    </div>
                  ))}
            ...
            <section className="settings-card settings-card-muted">
              <div className="settings-card-header">
                ...
                <span>{t("Java Requirements")}</span>
              </div>
              <p className="settings-tip">{t("Minecraft 1.17+ requires Java 17 or newer. Minecraft 1.20.5+ requires Java 21. Older versions work with Java 8. You can set a specific Java path per profile in the profile settings.")}
              </p>
            </section>
          </>
        )}
          ...
          <>
            <section className="settings-card" style={{ marginBottom: 24 }}>
              ...
              <div className="settings-row" style={{ alignItems: "center" }}>
                <div className="settings-row-content">
                  <div className="settings-row-title">{t("VelGrinor Launcher")}</div>
                  <div className="settings-row-description">{t("Version")} {appVersion ?? "—"}
                ...
                >
                  {checkingAppUpdate ? t("Checking...") : t("Check")}
                </button>
              </div>
              ...
              {appUpdate && (
                ...
                >
                  {downloadingAppUpdate ? t("Downloading...") : t("Download & Install")}
                </button>
              )}
            </section>
            ...
            <section className="settings-card" style={{ marginBottom: 24 }}>
              ...
              {updateResult && updateResult.updates.length > 0 && (
                <div className="settings-updates-list">
                  <div className="settings-updates-header">
                    <span>{updateResult.updates.length}{t("update")}{updateResult.updates.length !== 1 ? "s" : ""}{t("available")}</span>
                    <button className="btn btn-sm btn-primary" onClick={handleApplyAllUpdates}>{t("Update all")}
                    </button>
                  </div>
                    ...
                    return (
                      <div key={key} className="settings-update-item">
                        <div className="settings-update-info">
                          ...
                          {isPinned && <span className="badge badge-warning">{t("Pinned")}</span>}
                          {isManual && <span className="badge badge-muted">{t("Manual")}</span>}
              ...
              {updateResult && updateResult.updates.length === 0 && (
                <p className="settings-muted">{t("All content is up to date")}
                </p>
              )}

              {updateResult && updateResult.errors.length > 0 && (
                <p className="settings-error" style={{ marginTop: 12 }}>
                  {updateResult.errors.length}{t("error")}{updateResult.errors.length !== 1 ? "s" : ""}{t("during check")}
                </p>
              )}
            </section>
          </>
          ...
          <>
            <section className="settings-card" style={{ marginBottom: 24 }}>
              <div className="settings-card-header">
                ...
                <span>{t("VelGrinor Launcher")}</span>
              </div>

              <div className="about-info">
                <div className="about-logo">
                  <img src="/icon.png" alt="VelGrinor" width="64" height="64" style={{ borderRadius: 12 }} />
                </div>
                <div className="about-details">
                  <div className="about-name">{t("VelGrinor")}</div>
                  <div className="about-version">{t("Version")} {appVersion ?? "—"}</div>
                  <div className="about-description">{t("A minimal, clean, CLI-first Minecraft launcher focused on stability, reproducibility, and low duplication.")}
                  </div>
                </div>
              </div>
            </section>
              ...
              <section className="settings-card" style={{ marginBottom: 24 }}>
                ...
                <div className="about-system-info">
                  <div className="about-system-row">
                    <span className="about-system-label">{t("Platform")}</span>
                    <span className="about-system-value" style={{ textTransform: "capitalize" }}>{osInfo.platform}</span>
                  </div>
                  <div className="about-system-row">
                    <span className="about-system-label">{t("Architecture")}</span>
                    <span className="about-system-value">{osInfo.arch}</span>
                  </div>
                  <div className="about-system-row">
                    <span className="about-system-label">{t("OS Version")}</span>
            ...
            <section className="settings-card" style={{ marginBottom: 24 }}>
              ...
              <div className="about-links">
                <button className="btn btn-ghost" style={{ justifyContent: "flex-start", width: "100%" }} onClick={() => openUrl("https://github.com/Sqrilizz/VelGrinor")}>
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="currentColor">
                    <path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0016 8c0-4.42-3.58-8-8-8z" />
                  </svg>{t("VelGrinor on GitHub")}
                </button>
                <button className="btn btn-ghost" style={{ justifyContent: "flex-start", width: "100%" }} onClick={() => openUrl("https://sqrilizz.tech")}>
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                    <circle cx="8" cy="8" r="6.25" stroke="currentColor" strokeWidth="1.5" />
                    <path d="M1.75 8h12.5M8 1.75c1.7 1.7 2.55 3.78 2.55 6.25S9.7 12.55 8 14.25C6.3 12.55 5.45 10.47 5.45 8S6.3 3.45 8 1.75z" stroke="currentColor" strokeWidth="1.25" strokeLinecap="round" />
                  </svg>{t("Creator's Website")}
                </button>
              </div>
            </section>
            ...
            <section className="settings-card settings-card-muted">
              ...
              <p className="settings-tip">{t("Built with Rust, Tauri, and React. Minecraft is a trademark of Mojang Studios. This launcher is not affiliated with or endorsed by Mojang Studios or Microsoft.")}
              </p>
            </section>
          </>
        )}
