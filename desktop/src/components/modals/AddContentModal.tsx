...
export function AddContentModal({ open, kind, onClose, onAddFromLibrary }: AddContentModalProps) {
  ...
  const handleImport = async (paths?: string[]) => {
    ...
    setImporting(false);
    if (added > 0) {
      notify(t("Imported"), `Added ${added} ${getContentTypeLabel(kind).toLowerCase()}${added === 1 ? "" : "s"}`);
  ...
  const handleStoreInstall = async () => {
    ...
    try {
      ...
      notify(t("Installed"), `${selectedStoreItem.name} added to profile`);
      onClose();
    } catch (err) {
  ...
  return (
    <Modal open={open} onClose={onClose} title={t("Add {{type}}", { type: contentLabel })} className="modal-lg">
      <div className="add-content-modal">
        ...
        <input
          type="text"
          className="input"
          placeholder={source === "library" ? t("Search your library...") : t("Search {{type}}...", { type: `${contentLabel.toLowerCase()}s` })}
          value={search}
        ...
        >
          {isDragging && (
            <div className="add-content-drop-overlay">
              <span>{t("Drop files to import")}</span>
            </div>
          )}
          ...
          {!loading && source === "library" && libraryItems.length === 0 && (
            <div className="add-content-empty">
              <span className="text-muted">
                {search ? t("No matches found") : t("No {{type}} in library", { type: `${contentLabel.toLowerCase()}s` })}
              </span>
          ...
          {!loading && source === "store" && storeResults.length === 0 && (
            <div className="add-content-empty">
              <span className="text-muted">
                {search ? t("No compatible results found") : t("No popular content available")}
              </span>
            </div>
          )}
        ...
        <div className="modal-footer">
          ...
          >
            {installing ? t("Installing...") : t("Add to profile")}
          </button>
        </div>
      </div>
    </Modal>
