...
export function AccountDetailsModal({ open: isOpen, accountId, onClose }: AccountDetailsModalProps) {
  ...
  return (
    <Modal open={isOpen} onClose={onClose} className="modal-lg">
      <h2 className="modal-title">{info?.username ?? t("Account")}</h2>

      {loading && (
        <div style={{ textAlign: "center", padding: 40 }}>
          <div className="skin-viewer-loading" />
          <p style={{ marginTop: 16, color: "var(--text-secondary)" }}>{t("Loading account info...")}</p>
        </div>
      )}
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {tab === "skin" && (
              <div>
                ...
                {/* Library grid */}
                {loadingLibrary ? (
                  <p style={{ color: "var(--text-muted)", fontSize: 13, textAlign: "center", padding: 20 }}>{t("Loading skins...")}</p>
                ) : librarySkins.length === 0 ? (
                  <div style={{ textAlign: "center", padding: 24, background: "rgba(255,255,255,0.02)", borderRadius: 10, border: "1px solid var(--border-subtle)" }}>
                    ...
                    <p style={{ color: "var(--text-muted)", fontSize: 13, marginBottom: 12 }}>{t("No skins in library yet")}
                    </p>
                    <button
                      ...
                      disabled={uploading}
                    >
                      {uploading ? t("Uploading...") : t("Upload Your First Skin")}
                    </button>
                  </div>
                ) : (
                  <div
                ...
                {selectedLibrarySkin && (
                  <div style={{ marginTop: 12, padding: 12, background: "rgba(255,255,255,0.02)", borderRadius: 10, border: "1px solid var(--border-subtle)" }}>
                    ...
                    <button
                      ...
                      style={{ width: "100%" }}
                    >
                      {uploading ? t("Applying...") : t("Apply \"{{name}}\"", { name: selectedLibrarySkin.name })}
                    </button>
                  </div>
                )}
            ...
            {tab === "capes" && (
              <div>
                {(info.profile?.capes?.length ?? 0) === 0 ? (
                  ...
                  >
                    <p>{t("No capes available for this account.")}</p>
                    <p style={{ fontSize: 12, marginTop: 8, color: "var(--text-muted)" }}>{t("Capes are obtained through Minecraft events, purchases, or promotions.")}
                    </p>
                  </div>
                ) : (
                    ...
                    {(info.profile?.capes ?? []).map((cape: Cape) => (
                      ...
                      >
                        <div className="content-item-info">
                          ...
                          {cape.state === "ACTIVE" && (
                            <p style={{ color: "var(--accent-primary)" }}>{t("Currently active")}</p>
                          )}
                        </div>
      ...
      <div className="modal-footer">
        <button className="btn btn-secondary" onClick={onClose}>{t("Close")}
        </button>
      </div>
    </Modal>
  );
