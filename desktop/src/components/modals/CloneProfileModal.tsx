...
export function CloneProfileModal({ open, onClose, onSubmit }: CloneProfileModalProps) {
  ...
  return (
    <Modal open={open} onClose={onClose} title={t("Clone profile")}>
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        ...
        <Field label="New profile ID" error={errors.dst}>
          <input className={clsx("input", errors.dst && "input-error")} value={form.dst} onChange={(e) => setForm({ ...form, dst: e.target.value })} placeholder={t("my-modpack-copy")} />
        </Field>
