...
export function EditVersionModal({ open, onClose, mode }: EditVersionModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      notify(t("Profile updated"), `Changed to ${mcVersion}${loaderType ? ` with ${loaderType}` : ""}`);
      onClose();
    } catch (err) {
      notify(t("Failed to update profile"), String(err));
    } finally {
            ...
            {(loaderType === "quilt" || loaderType === "forge" || loaderType === "neoforge") && (
              <Field label={`${loaderType.charAt(0).toUpperCase() + loaderType.slice(1)} version`}>
                <input
                  ...
                  value={loaderVersion}
                  onChange={(e) => setLoaderVersion(e.target.value)}
                  placeholder={t("Enter version (e.g., 0.20.0)")}
                />
              </Field>
            )}
          </>
