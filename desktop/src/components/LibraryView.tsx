...
type LibraryCategory = "all" | "mod" | "modpack" | "resourcepack" | "shaderpack" | "skin";
...
export function LibraryView() {
  ...
  const [compatibilityFilter, setCompatibilityFilter] = useState<string | null>(null);
  ...
  const compatibilityTags = tags.filter((tag) => isSystemTag(tag.name));
  const projectIcons = useProjectIcons(items.map(getModrinthProjectId));
  ...
  const loadItems = useCallback(async () => {
    try {
      const filter: LibraryFilter = {
        ...
        tags: [selectedTagFilter, compatibilityFilter].filter((value): value is string => !!value).length > 0
          ? [selectedTagFilter, compatibilityFilter].filter((value): value is string => !!value)
          : undefined,
        limit: 100,
      };
      ...
      notify(t("Failed to load library"), String(err));
    }
  }, [category, search, selectedTagFilter, compatibilityFilter, notify]);
  ...
  const handleImportFile = async () => {
    const extensions = category === "all" ? ["jar", "zip", "mrpack", "png"] :
      category === "mod" ? ["jar"] :
      category === "modpack" ? ["mrpack", "zip"] :
    ...
    for (const path of paths) {
      try {
        ...
        if (errStr.includes("UNIQUE constraint")) {
          ...
          notify(t("Import failed"), errStr);
        }
      }
    }

    setImporting(false);
    notify(t("Import complete"), `Added ${added}, skipped ${skipped} duplicates`);
  ...
  const handleImportFolder = async () => {
    ...
    try {
      ...
      notify(t("Import complete"), `Added ${importResult.added}, skipped ${importResult.skipped}`);
      if (importResult.errors.length > 0) {
        console.error("Import errors:", importResult.errors);
      }
    } catch (err) {
      notify(t("Import failed"), String(err));
    }
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      notify(t("Sync complete"), `Found ${result.added} new items in store`);
    } catch (err) {
      notify(t("Sync failed"), String(err));
    }
    setImporting(false);
    await Promise.all([loadItems(), loadStats()]);
  };
  ...
  const handleAddToProfile = async (item: LibraryItem) => {
    if (!selectedProfileId) {
      notify(t("No profile selected"), "Select a profile first");
      return;
    }
    ...
    try {
      ...
      notify(t("Added to profile"), `${formatContentName(item.name)} added to ${selectedProfileId}`);
    } catch (err) {
      notify(t("Failed to add"), String(err));
    }
  };
  ...
  const handleDeleteItem = async (item: LibraryItem) => {
    try {
      ...
      setSelectedItem(null);
      await Promise.all([loadItems(), loadStats()]);
      notify(t("Removed"), `${formatContentName(item.name)} removed from library`);
    } catch (err) {
      notify(t("Failed to remove"), String(err));
    }
  };
  ...
  const handleBulkRemove = async () => {
    ...
    for (const id of selectedIds) {
      try {
        await invoke("library_delete_item_cmd", { id, deleteFile: false });
        removed += 1;
      } catch (err) {
        console.error("Bulk library removal failed:", err);
      }
    }
    ...
    setImporting(false);
    await Promise.all([loadItems(), loadStats()]);
    notify(t("Library updated"), `Removed ${removed} items`);
  };
  ...
  const getSourceUrl = (item: LibraryItem): string | null => {
    ...
    const typeMap: Record<string, { modrinth: string; curseforge: string }> = {
      ...
      modpack: { modrinth: "modpack", curseforge: "modpacks" },
    ...
    <div className="view-transition">
            ...
            {stats && cat !== "all" && (
              <span style={{ marginLeft: 6, opacity: 0.6, fontSize: 11 }}>
                ({cat === "mod" ? stats.mods_count :
                  cat === "modpack" ? stats.modpacks_count :
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16, alignItems: "center", position: "relative" }}>
        ...
        <select
          ...
          onChange={(event) => setCompatibilityFilter(event.target.value || null)}
          style={{ width: 180 }}
        >
          <option value="">{t("All compatibility")}</option>
          {compatibilityTags.map((tag) => (
            <option key={tag.id} value={tag.name}>
              {tag.name.startsWith("mc:") ? `Minecraft ${tag.name.slice(3)}` : `Loader: ${tag.name.slice(7)}`}
            </option>
          ))}
        </select>
        ...
        <div style={{ position: "relative" }}>
          ...
          {showTagDropdown && (
            <div className="sidebar-add-menu" style={{ right: 0, left: "auto", minWidth: 140 }}>
              <button onClick={() => { setSelectedTagFilter(null); setShowTagDropdown(false); }}>{t("All tags")}
              </button>
              {visibleTags.map((tag) => (
                <button
                  key={tag.id}
        ...
        <button className="btn btn-secondary" onClick={handleImportFile} disabled={importing}>{t("Import Files")}
        </button>
        <button className="btn btn-secondary" onClick={handleImportFolder} disabled={importing}>{t("Import Folder")}
        </button>
        <button className="btn btn-secondary" onClick={handleSync} disabled={importing}>{t("Sync")}
        </button>
        {selectedIds.size > 0 && (
          <>
            <button className="btn btn-secondary" onClick={() => setSelectedIds(new Set(items.map((item) => item.id)))}>{t("Select all")}
            </button>
            <button className="btn btn-danger" onClick={handleBulkRemove} disabled={importing}>{t("Remove selected (")}{selectedIds.size})
            </button>
          </>
        )}
      </div>
      ...
      <div style={{ display: "flex", gap: 24, flex: 1, minHeight: 0 }}>
        ...
        <div style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" }}>
          {loading && (
            <p style={{ color: "var(--text-muted)", fontSize: 13 }}>{t("Loading library...")}</p>
          )}
          ...
          {!loading && items.length === 0 && (
            <div className="empty-state-container">
              <div className="empty-state">
                ...
                <p style={{ marginBottom: 0 }}>{t("Import files or download content from the store to get started.")}</p>
              </div>
            </div>
          )}
          ...
          {items.map((item) => {
            ...
            if (item.content_type !== "skin") {
              return (
                <div key={item.id} className="library-selectable-row">
                  <button
                    className={clsx("library-select-check", selectedIds.has(item.id) && "active")}
                  ...
                  <ContentItemRow
                    ...
                    contentType={item.content_type === "mod" ? "mods" : item.content_type === "modpack" ? "modpacks" : item.content_type === "resourcepack" ? "resourcepacks" : "shaderpacks"}
              ...
              >
                ...
                <div className="content-item-main">
                  ...
                  <div className="content-item-meta">
                    <span className="content-meta-platform" style={{ color: "var(--text-muted)" }}>{t("Skin")}
                    </span>
                    {item.file_size && (
        ...
        {selectedItem && (
          ...
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10, fontSize: 13 }}>
              <div>
                <div style={{ color: "var(--text-muted)", fontSize: 11, textTransform: "uppercase", marginBottom: 2 }}>{t("Type")}
                </div>
              ...
              {selectedItem.file_name && (
                <div>
                  <div style={{ color: "var(--text-muted)", fontSize: 11, textTransform: "uppercase", marginBottom: 2 }}>{t("File")}
                  </div>
              ...
              {selectedItem.file_size && (
                <div>
                  <div style={{ color: "var(--text-muted)", fontSize: 11, textTransform: "uppercase", marginBottom: 2 }}>{t("Size")}
                  </div>
              ...
              {selectedItem.source_platform && (
                <div>
                  <div style={{ color: "var(--text-muted)", fontSize: 11, textTransform: "uppercase", marginBottom: 2 }}>{t("Source")}
                  </div>
                  {(() => {
              ...
              <div>
                <div style={{ color: "var(--text-muted)", fontSize: 11, textTransform: "uppercase", marginBottom: 2 }}>{t("Added")}
                </div>
              ...
              {selectedItem.used_by_profiles.length > 0 && (
                <div>
                  <div style={{ color: "var(--text-muted)", fontSize: 11, textTransform: "uppercase", marginBottom: 2 }}>{t("Used by")}
                  </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 20 }}>
              {selectedItem.content_type !== "skin" && selectedItem.content_type !== "modpack" && (
                <button
